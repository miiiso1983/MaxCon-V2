...
<!-- Customer Form -->
<form method="POST" action="{{ route('tenant.sales.customers.update', $customer) }}">
    @csrf
    @method('PUT')
    ...
    <div class="content-card" style="margin-bottom: 25px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            ...
            <div>
                ...
                <input type="text" name="commercial_register" value="{{ old('commercial_register', $customer->commercial_register) }}" 
                       style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;" 
    ...
    <div class="content-card" style="margin-bottom: 25px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            ...
            <div style="grid-column: 1 / -1;">
                ...
                <textarea name="address" style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; height: 80px;" placeholder="العنوان الكامل...">{{ old('address', $customer->address) }}</textarea>
                @error('address')
                    <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="country" required style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;">
                    <option value="SA" {{ old('country', $customer->country) === 'SA' ? 'selected' : '' }}>السعودية</option>
                    <option value="AE" {{ old('country', $customer->country) === 'AE' ? 'selected' : '' }}>الإمارات</option>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            ...
            <div>
                ...
                <select name="payment_terms" required style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;">
                    <option value="cash" {{ old('payment_terms', $customer->payment_terms) === 'cash' ? 'selected' : '' }}>نقداً</option>
                    <option value="credit_7" {{ old('payment_terms', $customer->payment_terms) === 'credit_7' ? 'selected' : '' }}>آجل 7 أيام</option>
                    <option value="credit_15" {{ old('payment_terms', $customer->payment_terms) === 'credit_15' ? 'selected' : '' }}>آجل 15 يوم</option>
                    <option value="credit_30" {{ old('payment_terms', $customer->payment_terms) === 'credit_30' ? 'selected' : '' }}>آجل 30 يوم</option>
                    <option value="credit_60" {{ old('payment_terms', $customer->payment_terms) === 'credit_60' ? 'selected' : '' }}>آجل 60 يوم</option>
                    <option value="credit_90" {{ old('payment_terms', $customer->payment_terms) === 'credit_90' ? 'selected' : '' }}>آجل 90 يوم</option>
                </select>
                @error('payment_terms')
                    <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="credit_limit" value="{{ old('credit_limit', $customer->credit_limit) }}" min="0" step="0.01" required 
                       style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;" 
                       placeholder="0.00">
                @error('credit_limit')
                    <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
            ...
            <div>
                ...
                <select name="currency" required style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;">
                    <option value="SAR" {{ old('currency', $customer->currency) === 'SAR' ? 'selected' : '' }}>ريال سعودي (SAR)</option>
                    <option value="AED" {{ old('currency', $customer->currency) === 'AED' ? 'selected' : '' }}>درهم إماراتي (AED)</option>
                    <option value="USD" {{ old('currency', $customer->currency) === 'USD' ? 'selected' : '' }}>دولار أمريكي (USD)</option>
                    <option value="EUR" {{ old('currency', $customer->currency) === 'EUR' ? 'selected' : '' }}>يورو (EUR)</option>
                </select>
                @error('currency')
                    <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                @enderror
    ...
    <div class="content-card" style="margin-bottom: 25px;">
        ...
        <div>
            ...
            <textarea name="notes" style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; height: 100px;" placeholder="ملاحظات إضافية عن العميل...">{{ old('notes', $customer->notes) }}</textarea>
            @error('notes')
                <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div style="display: flex; gap: 15px; justify-content: flex-end;">
        <a href="{{ route('tenant.sales.customers.show', $customer) }}" style="padding: 12px 24px; border: 2px solid #e2e8f0; border-radius: 8px; background: white; color: #4a5568; font-weight: 600; text-decoration: none; display: flex; align-items: center; gap: 8px;">
