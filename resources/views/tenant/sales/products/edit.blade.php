        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            ...
            <div>
                ...
                <select name="unit" required style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;">
                    ...
                    <option value="قرص" {{ old('unit', $product->unit) === 'قرص' ? 'selected' : '' }}>قرص</option>
                    <option value="كبسولة" {{ old('unit', $product->unit) === 'كبسولة' ? 'selected' : '' }}>كبسولة</option>
                    <option value="شراب" {{ old('unit', $product->unit) === 'شراب' ? 'selected' : '' }}>شراب</option>
                    <option value="حقنة" {{ old('unit', $product->unit) === 'حقنة' ? 'selected' : '' }}>حقنة</option>
                    <option value="مرهم" {{ old('unit', $product->unit) === 'مرهم' ? 'selected' : '' }}>مرهم</option>
                    <option value="قطرة" {{ old('unit', $product->unit) === 'قطرة' ? 'selected' : '' }}>قطرة</option>
                    <option value="بخاخ" {{ old('unit', $product->unit) === 'بخاخ' ? 'selected' : '' }}>بخاخ</option>
                    <option value="علبة" {{ old('unit', $product->unit) === 'علبة' ? 'selected' : '' }}>علبة</option>
                    <option value="زجاجة" {{ old('unit', $product->unit) === 'زجاجة' ? 'selected' : '' }}>زجاجة</option>
                </select>
                @error('unit')
                    <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                @enderror
    ...
    <div class="content-card" style="margin-bottom: 25px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
            ...
            <div>
                ...
                <input type="number" name="current_stock" value="{{ old('current_stock', $product->current_stock) }}" min="0" required 
                       style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px;" 
                       placeholder="0">
                @error('current_stock')
                    <div style="color: #f56565; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
