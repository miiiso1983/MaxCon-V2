...
@section('page-title', 'تعديل طلب الشراء: ' . $purchaseRequest->request_number)
@section('page-description', 'تعديل طلب الشراء الداخلي')
    ...
    <div style="position: relative; z-index: 2;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <div style="display: flex; align-items: center; margin-bottom: 15px;">
                    ...
                    <div>
                        ...
                        <p style="font-size: 18px; margin: 5px 0 0 0; opacity: 0.9;">
                            {{ $purchaseRequest->request_number }} - {{ $purchaseRequest->title }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 15px;">
                <a href="{{ route('tenant.purchasing.purchase-requests.show', $purchaseRequest) }}" style="background: rgba(255,255,255,0.2); color: white; padding: 15px 25px; border-radius: 15px; text-decoration: none; font-weight: 600; display: flex; align-items: center; gap: 10px; backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.3);">
...
<div class="content-card">
    <form method="POST" action="{{ route('tenant.purchasing.purchase-requests.update', $purchaseRequest) }}" id="purchaseRequestForm">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 30px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                ...
                <div>
                    ...
                    <select name="priority" required style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px;">
                        ...
                        <option value="low" {{ old('priority', $purchaseRequest->priority) === 'low' ? 'selected' : '' }}>منخفضة</option>
                        <option value="medium" {{ old('priority', $purchaseRequest->priority) === 'medium' ? 'selected' : '' }}>متوسطة</option>
                        <option value="high" {{ old('priority', $purchaseRequest->priority) === 'high' ? 'selected' : '' }}>عالية</option>
                        <option value="urgent" {{ old('priority', $purchaseRequest->priority) === 'urgent' ? 'selected' : '' }}>عاجل</option>
                    </select>
                    @error('priority')
                        <div style="color: #ef4444; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                    @enderror
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="date" name="required_date" value="{{ old('required_date', $purchaseRequest->required_date->format('Y-m-d')) }}" required 
                           style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px;"
                ...
                <div>
                    ...
                    <input type="number" name="estimated_total" value="{{ old('estimated_total', $purchaseRequest->estimated_total) }}" step="0.01" min="0"
                           style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px;"
                           placeholder="0.00">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="description" rows="3" 
                          ...
                          placeholder="أدخل وصف تفصيلي للطلب">{{ old('description', $purchaseRequest->description) }}</textarea>
                @error('description')
                    <div style="color: #ef4444; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="justification" rows="2" 
                          style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px; resize: vertical;"
                          placeholder="أدخل مبرر الحاجة لهذا الطلب">{{ old('justification', $purchaseRequest->justification) }}</textarea>
                @error('justification')
                    <div style="color: #ef4444; font-size: 14px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                <input type="checkbox" name="is_urgent" id="is_urgent" value="1" {{ old('is_urgent', $purchaseRequest->is_urgent) ? 'checked' : '' }}
        ...
        <div style="margin-bottom: 30px;">
            <h3 style="font-size: 20px; font-weight: 700; color: #2d3748; margin-bottom: 20px; display: flex; align-items: center;">
                <i class="fas fa-cog" style="color: #f59e0b; margin-left: 10px;"></i>
                معلومات إضافية
            </h3>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="text" name="budget_code" value="{{ old('budget_code', $purchaseRequest->budget_code) }}" 
                           style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px;"
                ...
                <div>
                    ...
                    <input type="text" name="cost_center" value="{{ old('cost_center', $purchaseRequest->cost_center) }}" 
                           style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px;"
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="special_instructions" rows="3" 
                          style="width: 100%; padding: 12px; border: 2px solid #e2e8f0; border-radius: 8px; font-size: 16px; resize: vertical;"
                          placeholder="أدخل أي تعليمات خاصة للطلب">{{ old('special_instructions', $purchaseRequest->special_instructions) }}</textarea>
        ...
        <div style="display: flex; gap: 15px; justify-content: flex-end; padding-top: 20px; border-top: 1px solid #e2e8f0;">
            <a href="{{ route('tenant.purchasing.purchase-requests.show', $purchaseRequest) }}" style="background: #6b7280; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; display: flex; align-items: center; gap: 8px;">
                <i class="fas fa-times"></i>
                إلغاء
            </a>
            <button type="submit" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white; padding: 12px 24px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px;">
...
// Load existing items
const existingItems = @json($purchaseRequest->items);
        ...
        <div class="item-row" style="background: #f8fafc; padding: 20px; border-radius: 12px; margin-bottom: 15px; border: 1px solid #e2e8f0;">
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                <div>
                    ...
                    <input type="text" name="items[${itemIndex}][item_name]" value="${itemData.item_name}" required 
                ...
                <div>
                    ...
                    <input type="number" name="items[${itemIndex}][quantity]" value="${itemData.quantity}" required step="0.01" min="0.01"
                           style="width: 100%; padding: 10px; border: 2px solid #e2e8f0; border-radius: 6px; font-size: 14px;"
                           placeholder="0">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="items[${itemIndex}][estimated_price]" value="${itemData.estimated_price}" step="0.01" min="0"
                           style="width: 100%; padding: 10px; border: 2px solid #e2e8f0; border-radius: 6px; font-size: 14px;"
                           placeholder="0.00">
                </div>
            </div>
            ...
            <div style="margin-bottom: 15px;">
                ...
                <input type="text" name="items[${itemIndex}][unit]" value="${itemData.unit}"
                       style="width: 100%; padding: 10px; border: 2px solid #e2e8f0; border-radius: 6px; font-size: 14px;"
            ...
            <div>
                ...
                <textarea name="items[${itemIndex}][specifications]" rows="2"
                          ...
                          placeholder="أدخل المواصفات المطلوبة">${itemData.specifications}</textarea>
            </div>
        </div>
    `;
