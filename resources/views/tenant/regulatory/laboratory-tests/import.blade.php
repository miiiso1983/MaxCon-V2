...
@section('title', 'استيراد الفحوصات المخبرية من Excel')
    ...
    <div style="background: rgba(255,255,255,0.95); border-radius: 20px; padding: 30px; margin-bottom: 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(10px);">
        <div style="display: flex; align-items: center; justify-content: between; margin-bottom: 20px;">
            <div style="display: flex; align-items: center;">
                ...
                <div>
                    <h1 style="color: #2d3748; margin: 0; font-size: 32px; font-weight: 700;">استيراد الفحوصات المخبرية من Excel</h1>
                    <p style="color: #718096; margin: 5px 0 0 0; font-size: 16px;">رفع ملف Excel أو CSV لاستيراد بيانات الفحوصات المخبرية</p>
                </div>
            </div>
            <div style="display: flex; gap: 15px;">
                <a href="{{ route('tenant.inventory.regulatory.laboratory-tests.download-template') }}" style="background: linear-gradient(135deg, #48bb78 0%, #38a169 100%); color: white; padding: 15px 25px; border: none; border-radius: 15px; font-weight: 600; display: flex; align-items: center; gap: 10px; text-decoration: none;">
    ...
    <div style="background: rgba(255,255,255,0.95); border-radius: 20px; padding: 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(10px);">
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 15px; padding: 25px; margin-bottom: 30px;">
            ...
            <ul style="margin: 0; padding-right: 20px; line-height: 1.8;">
                ...
                <li>تأكد من أن الحقول المطلوبة (اسم الفحص، اسم المنتج، رقم الدفعة) مملوءة</li>
                <li>استخدم التواريخ بصيغة YYYY-MM-DD (مثل: 2024-01-15)</li>
                <li>استخدم القيم المحددة لنوع الفحص وحالة الفحص كما في النموذج</li>
                <li>حجم الملف يجب أن يكون أقل من 2 ميجابايت</li>
            </ul>
        </div>

        <!-- Upload Form -->
        <form action="{{ route('tenant.inventory.regulatory.laboratory-tests.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
            @csrf
            
            <div style="text-align: center; padding: 40px 20px;">
                <div id="dropZone" style="border: 3px dashed #667eea; border-radius: 15px; padding: 60px 20px; background: rgba(102, 126, 234, 0.05); transition: all 0.3s; cursor: pointer;"
        ...
        <div style="margin-top: 40px; padding-top: 30px; border-top: 2px solid #e2e8f0;">
            <h3 style="color: #2d3748; margin: 0 0 20px 0; font-size: 20px; font-weight: 700;">
                <i class="fas fa-table" style="margin-left: 10px; color: #667eea;"></i>
                ...
                <table style="width: 100%; border-collapse: collapse; background: white;">
                    <thead>
                        <tr style="background: #f7fafc;">
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">اسم الفحص</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">نوع الفحص</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">اسم المنتج</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">رقم الدفعة</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">اسم المختبر</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">حالة الفحص</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">فحص جودة الأقراص</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">مراقبة الجودة</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">باراسيتامول 500 مجم</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">BATCH001</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">مختبر الجودة المركزي</td>
            ...
            <!-- Valid Values -->
            <div style="margin-top: 20px; padding: 20px; background: rgba(102, 126, 234, 0.05); border-radius: 10px;">
                <h4 style="color: #2d3748; margin: 0 0 15px 0; font-weight: 700;">القيم المقبولة:</h4>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                    <div>
                        <strong style="color: #667eea;">نوع الفحص:</strong>
                        <ul style="margin: 5px 0 0 20px; color: #4a5568; font-size: 14px;">
                            <li>مراقبة الجودة</li>
                            <li>اختبار الثبات</li>
                            <li>فحص ميكروبيولوجي</li>
                            <li>فحص كيميائي</li>
                            <li>فحص فيزيائي</li>
                            <li>التكافؤ الحيوي</li>
                        </ul>
                    </div>
                    <div>
                        <strong style="color: #667eea;">حالة الفحص:</strong>
                        <ul style="margin: 5px 0 0 20px; color: #4a5568; font-size: 14px;">
                            <li>في الانتظار</li>
                            <li>قيد التنفيذ</li>
                            <li>مكتمل</li>
...
function dragLeaveHandler(ev) {
    document.getElementById('dropZone').style.borderColor = '#667eea';
    document.getElementById('dropZone').style.background = 'rgba(102, 126, 234, 0.05)';
}
...
function fileSelected(input) {
    ...
    if (file) {
        ...
        document.getElementById('submitBtn').style.background = 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)';
