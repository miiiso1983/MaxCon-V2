...
@section('title', 'استيراد التفتيشات من Excel')
...
<div style="padding: 30px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); min-height: 100vh;">
    ...
    <div style="background: rgba(255,255,255,0.95); border-radius: 20px; padding: 30px; margin-bottom: 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(10px);">
        <div style="display: flex; align-items: center; justify-content: between; margin-bottom: 20px;">
            <div style="display: flex; align-items: center;">
                ...
                <div>
                    <h1 style="color: #2d3748; margin: 0; font-size: 32px; font-weight: 700;">استيراد التفتيشات من Excel</h1>
                    <p style="color: #718096; margin: 5px 0 0 0; font-size: 16px;">رفع ملف Excel أو CSV لاستيراد بيانات التفتيشات التنظيمية</p>
                </div>
            </div>
            <div style="display: flex; gap: 15px;">
                <a href="{{ route('tenant.inventory.regulatory.inspections.download-template') }}" style="background: linear-gradient(135deg, #48bb78 0%, #38a169 100%); color: white; padding: 15px 25px; border: none; border-radius: 15px; font-weight: 600; display: flex; align-items: center; gap: 10px; text-decoration: none;">
    ...
    <div style="background: rgba(255,255,255,0.95); border-radius: 20px; padding: 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(10px);">
        ...
        <!-- Instructions -->
        <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border-radius: 15px; padding: 25px; margin-bottom: 30px;">
            ...
            <ul style="margin: 0; padding-right: 20px; line-height: 1.8;">
                ...
                <li>تأكد من أن الحقول المطلوبة (عنوان التفتيش، اسم المفتش، الجهة) مملوءة</li>
                <li>استخدم التواريخ بصيغة YYYY-MM-DD (مثل: 2024-01-15)</li>
                <li>استخدم القيم المحددة لنوع التفتيش وحالة التفتيش كما في النموذج</li>
                <li>حجم الملف يجب أن يكون أقل من 2 ميجابايت</li>
            </ul>
        </div>

        <!-- Upload Form -->
        <form action="{{ route('tenant.inventory.regulatory.inspections.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
            @csrf
            
            <div style="text-align: center; padding: 40px 20px;">
                <div id="dropZone" style="border: 3px dashed #f093fb; border-radius: 15px; padding: 60px 20px; background: rgba(240, 147, 251, 0.05); transition: all 0.3s; cursor: pointer;"
                    ...
                    <div style="font-size: 64px; color: #f093fb; margin-bottom: 20px;">
        ...
        <div style="margin-top: 40px; padding-top: 30px; border-top: 2px solid #e2e8f0;">
            <h3 style="color: #2d3748; margin: 0 0 20px 0; font-size: 20px; font-weight: 700;">
                <i class="fas fa-table" style="margin-left: 10px; color: #f093fb;"></i>
                ...
                <table style="width: 100%; border-collapse: collapse; background: white;">
                    <thead>
                        <tr style="background: #f7fafc;">
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">عنوان التفتيش</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">نوع التفتيش</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">اسم المفتش</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">الجهة المفتشة</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">حالة التفتيش</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">تفتيش دوري للجودة</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">روتيني</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">د. أحمد محمد</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">وزارة الصحة العراقية</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">مكتمل</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <!-- Valid Values -->
            <div style="margin-top: 20px; padding: 20px; background: rgba(240, 147, 251, 0.05); border-radius: 10px;">
                <h4 style="color: #2d3748; margin: 0 0 15px 0; font-weight: 700;">القيم المقبولة:</h4>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                    <div>
                        <strong style="color: #f093fb;">نوع التفتيش:</strong>
                        <ul style="margin: 5px 0 0 20px; color: #4a5568; font-size: 14px;">
                            <li>روتيني</li>
                            <li>شكوى</li>
                            <li>متابعة</li>
                            <li>ما قبل الموافقة</li>
                            <li>ما بعد التسويق</li>
                        </ul>
                    </div>
                    <div>
                        <strong style="color: #f093fb;">حالة التفتيش:</strong>
                        <ul style="margin: 5px 0 0 20px; color: #4a5568; font-size: 14px;">
                            <li>مجدول</li>
                            <li>قيد التنفيذ</li>
                            <li>مكتمل</li>
                    ...
                    <div>
                        <strong style="color: #f093fb;">تقييم الامتثال:</strong>
                        <ul style="margin: 5px 0 0 20px; color: #4a5568; font-size: 14px;">
                            <li>ممتاز</li>
                            <li>جيد</li>
                            <li>مرضي</li>
...
function dragLeaveHandler(ev) {
    document.getElementById('dropZone').style.borderColor = '#f093fb';
    document.getElementById('dropZone').style.background = 'rgba(240, 147, 251, 0.05)';
}
...
function fileSelected(input) {
    ...
    if (file) {
        ...
        document.getElementById('submitBtn').style.background = 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)';
