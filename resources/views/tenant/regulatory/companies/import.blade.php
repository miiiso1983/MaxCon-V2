...
@section('title', 'استيراد الشركات من Excel')
    ...
    <div style="background: rgba(255,255,255,0.95); border-radius: 20px; padding: 30px; margin-bottom: 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(10px);">
        <div style="display: flex; align-items: center; justify-content: between; margin-bottom: 20px;">
            <div style="display: flex; align-items: center;">
                ...
                <div>
                    <h1 style="color: #2d3748; margin: 0; font-size: 32px; font-weight: 700;">استيراد الشركات من Excel</h1>
                    <p style="color: #718096; margin: 5px 0 0 0; font-size: 16px;">رفع ملف Excel أو CSV لاستيراد بيانات الشركات</p>
                </div>
            </div>
            <div style="display: flex; gap: 15px;">
                <a href="{{ route('tenant.inventory.regulatory.companies.download-template') }}" style="background: linear-gradient(135deg, #48bb78 0%, #38a169 100%); color: white; padding: 15px 25px; border: none; border-radius: 15px; font-weight: 600; display: flex; align-items: center; gap: 10px; text-decoration: none;">
    ...
    <div style="background: rgba(255,255,255,0.95); border-radius: 20px; padding: 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); backdrop-filter: blur(10px);">
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 15px; padding: 25px; margin-bottom: 30px;">
            ...
            <ul style="margin: 0; padding-right: 20px; line-height: 1.8;">
                ...
                <li>تأكد من أن الحقول المطلوبة (اسم الشركة، رقم التسجيل، رقم الترخيص) مملوءة</li>
                <li>تأكد من أن أرقام التسجيل والتراخيص غير مكررة</li>
        ...
        <!-- Upload Form -->
        <form action="{{ route('tenant.inventory.regulatory.companies.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
            @csrf
            
            <div style="text-align: center; padding: 40px 20px;">
                <div id="dropZone" style="border: 3px dashed #4facfe; border-radius: 15px; padding: 60px 20px; background: rgba(79, 172, 254, 0.05); transition: all 0.3s; cursor: pointer;"
                    ...
                    <div style="font-size: 64px; color: #4facfe; margin-bottom: 20px;">
        ...
        <div style="margin-top: 40px; padding-top: 30px; border-top: 2px solid #e2e8f0;">
            <h3 style="color: #2d3748; margin: 0 0 20px 0; font-size: 20px; font-weight: 700;">
                <i class="fas fa-table" style="margin-left: 10px; color: #4facfe;"></i>
                ...
                <table style="width: 100%; border-collapse: collapse; background: white;">
                    <thead>
                        <tr style="background: #f7fafc;">
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">اسم الشركة</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">رقم التسجيل</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">رقم الترخيص</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">نوع الترخيص</th>
                            <th style="padding: 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600; color: #2d3748;">الجهة التنظيمية</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">شركة الأدوية المتقدمة</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">REG001</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">LIC001</td>
                            <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; color: #4a5568;">تصنيع</td>
...
function dragLeaveHandler(ev) {
    document.getElementById('dropZone').style.borderColor = '#4facfe';
    document.getElementById('dropZone').style.background = 'rgba(79, 172, 254, 0.05)';
}
...
function fileSelected(input) {
    ...
    if (file) {
        ...
        document.getElementById('submitBtn').style.background = 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)';
