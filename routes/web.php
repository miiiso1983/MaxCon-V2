...
Route::middleware(['auth', 'tenant'])->prefix('tenant')->name('tenant.')->group(function () {
    ...
    Route::prefix('sales')->name('sales.')->group(function () {
        ...
        Route::prefix('targets')->name('targets.')->group(function () {
            ...
            // Health check for tenant context (controller method)
            Route::get('reports/health/check', [SalesTargetController::class, 'health'])->name('reports.health');
...
Route::middleware(['auth','tenant'])->prefix('tenant')->name('tenant.')->group(function(){
    ...
    // Secure streaming of receipt PDFs by payment id (works even if public/storage symlink broken)
    Route::get('/receipts/payment/{payment}', function (\App\Models\InvoicePayment $payment) {
        $user = auth()->user();
