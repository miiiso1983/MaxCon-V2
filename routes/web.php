...
Route::prefix('test')->name('test.')->group(function () {
    ...
    Route::get('tenant-debug', function () {
        try {
            ...
            $tenant1 = \App\Models\Tenant::where('domain', $host)->first();

            // Try without www
            $domainWithoutWww = str_starts_with($host, 'www.') ? substr($host, 4) : $host;
            $tenant2 = \App\Models\Tenant::where('domain', $domainWithoutWww)->first();

            // Check if tenant middleware is working
            $tenantFromApp = app('tenant') ?? null;

            return response()->json([
                'host' => $host,
                'domain_without_www' => $domainWithoutWww,
                'tenant_direct_match' => $tenant1 ? $tenant1->toArray() : null,
                'tenant_without_www' => $tenant2 ? $tenant2->toArray() : null,
                'tenant_from_app' => $tenantFromApp ? $tenantFromApp->toArray() : null,
    ...
    // Test sales targets with manual tenant setup
    Route::get('sales-targets-manual', function () {
        try {
            // Manual tenant setup
            ...
            if (!$user) {
                return 'No user found for tenant';
            }
            auth()->login($user);
            ...
            return 'Success! Controller executed. Response type: ' . get_class($response);

        } catch (\Exception $e) {
            return 'Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine();
        }
    })->name('sales-targets-manual');

    // Test view directly with minimal data
    Route::get('test-view-direct', function () {
        try {
            ...
            return view('tenant.sales.targets.reports', [
                'targets' => $targets,
    ...
    // Test the exact same route as the original but without middleware
    Route::get('sales-targets-no-middleware', function () {
        try {
            // Manual setup
            $tenant = \App\Models\Tenant::first();
            app()->instance('tenant', $tenant);
            ...
            // Use the exact same controller method
            $request = new \Illuminate\Http\Request();
            $controller = new \App\Http\Controllers\Tenant\SalesTargetController();
    ...
    })->name('sales-targets-no-middleware');
});
